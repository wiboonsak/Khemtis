...
function   data_book(){
      $sql="SELECT  * FROM tbl_gp_booking";
      return $this->db->query($sql)->result();
}

function update_commp_data($boat,$car,$hotel,$rest,$tran,$client_id){

          if($hotel>0){
            $data = array("comm_price"=>$hotel);
...
function Sa_Up_data($allData=NULL,$comm=NULL,$data_service=NULL,$id_row=NULL,$id_update=NULL){
...
function get_service_comm($ty,$clid){
 if($ty=='HOTEL'){
       $hotel_comm = 0;
       $sql = "SELECT comm_price FROM tbl_hotels WHERE client_id=$clid";
       $variable1 = $this->db->query($sql)->result();
       foreach ($variable1 as $value1) {$hotel_comm = $value1->comm_price;}
       return $hotel_comm;
 }else{
      return 0;
 }
...
// show Data Client User
 public function show_client_service(){
$sql = "SELECT 
tbl_users.user_id,
...
GROUP_CONCAT(tbl_services.service_cd ORDER BY tbl_services.service_cd) as tbl_all_sevice
FROM `tbl_clients` 
...
tbl_clients.contact_person,
tbl_clients.contact_phone 
order by tbl_clients.client_id asc 
";
      return $this->db->query($sql)->result();
 }
