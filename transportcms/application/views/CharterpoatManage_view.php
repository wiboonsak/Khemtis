...
<div class="content">
                    <div class="container-fluid">
                         <h4 style="padding-left: 15px;">Charter Poat Manage</h4>
					<div class="card-box">
  ...
  <div class="card-box"><div style="display: flex; flex-wrap: wrap;">
			<div class="col-lg-6 p-t-10"> 
                 ...
                 <select class="form-control" id="begin_place_id" name="begin_place_id" >
						        ...
						        <?php 
                                                $listPlace = $this->Charterpoat_model->get_placeData();
					...
					<div class="form-group row">
                        ...
                        <div class="col-md-9 col-sm-12">						
							...
							<?php  $idclient = $this->session->userdata('client_user_id');
                                if($tran_id !=''){
                                $y = 'Y';
                                $listTransport = $this->Charterpoat_model->listTransport($y,$idclient);
        ...
        var formdata = new FormData();
         ...
         <?php } ?>
           ...
           formdata.append('destination_place_id',$('#destination_place_id').val());
                    $.ajax({
                        url: "<?php echo base_url('Charterpoat/insertUpdate_route')?>", // point to server-side controller method
                        ...
                        success: function (data) {
                        	if(data != 0 ){
                             ...
                             setTimeout(function(){ window.location.href = "<?php echo base_url('Charterpoat/CharterpoatDetail/')?>"+data; }, 2000);
                            }
                        },
...
//		}).then(function () {
   ...
   function  addpriceLand(tran_id) {
	...
	var pricelandID = $('#pricelandID').val();
        ...
        $.post('<?php echo base_url('Charterpoat/Addpriceland') ?>', {transport:transport,price:price,tran_id:dataID,pricelandID:pricelandID}, function (data) {
           	if(data != 0 ){
                      show_msg("Save Success",1);
                      $('#pricelandID').val('');
                      $('#transport').val('');
   ...
   function  loadland() {
        var dataID = $('#dataID').val();
        $.post('<?php echo base_url('Charterpoat/loadland') ?>', {dataID:dataID}, function (data) {
            $('#showland').html(data);
        });
    }
     //---------------------------------
	...
	function delete_data(dataID,table){  
             if (confirm("Want to delete this data ?")) {
                 $.post('<?php echo base_url('Charterpoat/deleteData1')?>' , { dataID : dataID , table : table }, 
		function(data){  
		if(data==1){ 
                	show_msg("Save Success",1);
                        loadland();		
