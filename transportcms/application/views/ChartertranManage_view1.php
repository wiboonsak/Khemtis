    ...
    <form id="form_route" class="form-horizontal" enctype="multipart/form-data">   
        ...
        if ($tran_id != 0) {
            ...
            foreach ($get_chartertran as $value_chartertran) {
                $a_lang = array();
                $val_tran_id = $value_chartertran->tran_id;
                $t_active = $value_chartertran->rout_active;
                array_push(
                        $a_lang,
                        $value_chartertran->lang,
                        $value_chartertran->route_name,
                        $value_chartertran->begin_place_id,
                        $value_chartertran->destination_place_id
                );
...
<?php } ?>
                    ...
                    <div class="panel-body">
                            ...
                            foreach ($results_lang as $value_bx) {
                                ...
                                for ($lg = 0; $lg < (count($txt_lang)); $lg++) {
                                    if (isset($txt_lang[$lg][1]) && ($txt_lang[$lg][0] == $fld_valu)) {
                                        ...
                                        $charter_name = $txt_lang[$lg][1];
                                        $begin_place_id = $txt_lang[$lg][2];
                                        $destination_place_id = $txt_lang[$lg][3];
                                    }
                                }
                                $get_place = $this->Chartertran_model->get_place($fld_valu);
                                ?>
                                        ...
                                        <div class="col-sm-12" >
                                            <div class="card-box">
                                                <div class="col-lg-12 p-t-10" style="border:0px;border-width:1px;border-style: solid;border-color:#cccccc;padding-bottom:5px;border-radius:5px;background-color:#d4d4d4;"> 
                                                    <font style="font-size: 18px;font-weight: bold;color:#6f6f6f;">Charter Transport Details <?php echo $naem_lang ?></font>
                                                </div>
                                                <br>
                                                <br>
						...
						<ul class="nav nav-tabs" style="padding-left: 15px;padding-right: 15px;">                            
                            <li class="nav-item">
                                <a href="#profile" data-toggle="tab" aria-expanded="true" class="nav-link active" onclick="opendiv()">
                                   <i class="fa fa-file-text-o"></i> Route Data
                                </a>
                            </li>							
							<?php if($tran_id !='0'){ ?>
							<li class="nav-item">
                                                            <a href="#route" data-toggle="tab" aria-expanded="false" class="nav-link" onclick="closediv()">
                        ...
                        <div class="tab-pane show active" id="profile">
							<div class="form-group row">
                              ...
                              <div class="col-md-9 col-sm-12">
                                  <input type="text" id="name<?php echo $fld_valu ?>" name="name<?php echo $fld_valu ?>" class="form-control" value="<?php echo $charter_name?>" >
                              </div>
                            </div>
							...
							<div class="form-group row">
                               ...
                               <div class="col-md-9 col-sm-12">
                                  <select class="form-control" id="begin_place_id<?php echo $fld_valu ?>" name="begin_place_id<?php echo $fld_valu ?>"  ><!--onchange="placedataupdate(this.value)"-->
                                     <option value="">-- Select --</option>
					 <?php foreach($get_place as $listPlace2){ ?> 
									  
									  
                                     <option value="<?php echo $listPlace2->place_id?>" <?php if($begin_place_id == $listPlace2->place_id){echo 'selected';}?>><?php echo $listPlace2->place_title?></option>
									  <?php } ?>
							...
							<div class="form-group row">
                               ...
                               <div class="col-md-9 col-sm-12" id="div_destination">
                                  <select class="form-control" id="destination_place_id<?php echo $fld_valu ?>" name="destination_place_id<?php echo $fld_valu ?>">
                                     <option value="">-- Select --</option>
                                    <?php foreach($get_place as $listPlace3){ ?> 				 
									  
									  
									  <option value="<?php echo $listPlace3->place_id?>" <?php if($destination_place_id == $listPlace3->place_id){echo 'selected';}?>><?php echo $listPlace3->place_title?></option>
									 <?php } ?> 
				...
				<?php  } ?>			
                                                ...
                                                <div class="form-group row" id="divsubmit">
...
function submit_data() {
	...
	var formdata = $('#form_route').serialize();
		$.post("<?php echo base_url('Chartertransport/insertUpdate_van')?>", { formdata : formdata } ,
     function(result){ 
      //alert(result);
		   if(result !=0){ 
			var link = '<?php echo base_url('Chartertransport/ChartertransportDetail/')?>'+result;   
