...
class TransportContact extends CI_Controller {
  ...
  public function index($tran_id =NULL){	
	...
	$data["get_tbl_transport_profile"]= $this->Van_model->get_tbl_transport_profile($idclient);
	$data["idclient"] = $idclient;	
	//$data["get_boat"]= $this->Boat_model->get_boat_detail($boat_id);
	$this->load->view('Header',$data);// Load Heder And Script Top Page
	//$this->load->view('Home');
	$this->load->view('TransportContact_view',$data);
...
public function insert_profile(){
    ...
    $get_formdata = array();
 	parse_str($formdata, $get_formdata);
    $Result = $this->Van_model->Add_update_profile($get_formdata);
	echo $Result;
