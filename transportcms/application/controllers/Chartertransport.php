...
class Chartertransport extends CI_Controller {
...
$tran_id = 0;
	...
	//$this->load->view('Home');
	$this->load->view('ChartertranManage_view',$data);
  ...
  public function ChartertransportDetail($tran_id =NULL){
  	  $idclient = $this->session->userdata('client_user_id');
	  $data["tokengenered"] =$this->session->userdata('token_sesion');
	$data["tran_id"] = $tran_id; 
	$data["results_service"] = $this->Hotel_model->get_service($idclient);
	$data["results_lang"] = $this->Chartertran_model->get_lang();	
	$data["get_chartertran"]= $this->Chartertran_model->get_chartertran_detail($tran_id);
 ...
 public function insertUpdate_route(){
   ...
   $destination_place_id = $_POST["destination_place_id"];
      $recrow = $this->Chartertran_model->get_lang();
      $route_id = $this->Chartertran_model->Add_update_dataroute($idc,$val_route_active,$val_route_id,$begin_place_id,$destination_place_id);

if($route_id>0){
	     foreach ($recrow as  $value) {
         $lg = $value->fld_valu;
         $name = $_POST["name".$lg];
         $resultData = $this->Chartertran_model->Add_up_details_langroute($name,$lg,$route_id);
   }
}
echo $route_id;
}
  //---------------------------------------
	...
	public function Addpriceland(){
        ...
        $pricelandID = $this->input->post('pricelandID');
            $result = $this->Chartertran_model->insertpriceLand($transport,$price,$landID,$pricelandID);
    ...
    public function loadland() {
        ...
        $pricelandData = $this->Chartertran_model->loadland($dataID);
        ?>
<table class="table table-striped table-bordered table-hover table-checkable order-column full-width" id="example4" >
        ...
        foreach ($pricelandData->result() as $pricelandData2) {
            $listTransport = $this->Chartertran_model->listTransport($y,$idclient,$pricelandData2->transport_id);
	...
	public function deleteData1(){  
		...
		$Result = $this->Chartertran_model->deleteData($dataID,$table);
		echo $Result;	
	}
          //---------------------------	 
			...
			$this->db->where('charterboat_id', $dataID);	 		
                        if($this->db->delete('tbl_chartertransport_detail')){
                            $this->db->where('id', $dataID);
                             if($this->db->delete('tbl_chartertransport')){
                            $pass = '1';
                            }else{
                            $pass = '0';
                        }
