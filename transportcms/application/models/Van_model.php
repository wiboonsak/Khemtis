...
function get_tbl_transport_profile($client_id){
  $sql = "SELECT * FROM tbl_transport_profile WHERE client_id = $client_id";
...
//////////////////////////////////////////
function Add_update_profile($allData=NULL){
 $ch_txt = "";
    $results_lang = $this->get_lang();
    foreach($results_lang as $vldata){
        $lg = trim($vldata->fld_valu);

  $idc = $allData["val_tran_id"];
  if($this->ch_tr_prof($idc,$lg)==0){
  $data = array(
    "client_id" => $idc,
    "trn_name" => $allData["trn_name$lg"],   
    "trn_addr" => $allData["trn_addr$lg"],
    "trn_polic" => $allData["trn_polic_val$lg"],
    "trn_keyword" => $allData["trn_keyword"],
    "trn_phone" => $allData["trn_phone"],
    "trn_email" => $allData["trn_email"],
    "lang" => $lg,
    "comm_price" =>0
  );         
  if($this->db->insert('tbl_transport_profile', $data)){  $ch_txt="save"; }
  }else{
    $data = array(
    "trn_name" => $allData["trn_name$lg"],   
    "trn_addr" => $allData["trn_addr$lg"],
  ...
  $this->db->where('client_id',$idc);
  $this->db->where('lang',$lg);
  if($this->db->update('tbl_transport_profile', $data)){  $ch_txt="save"; }    
  }
}
  return $ch_txt;
}

function ch_tr_prof($idc,$lg){
$query = $this->db->query("SELECT count(*) as cnt_r from tbl_transport_profile WHERE client_id=$idc AND lang = '$lg'");
$row = $query->row();
if (isset($row)){
   return $row->cnt_r;
  }else{
