<?php
class Charterpoat_model extends CI_Model{
...
function ch_tran_distich($id1){
  $sql = "SELECT * FROM tbl_charterboat WHERE id = '$id1'";
...
function get_chartertran_list($client_id=NULL){
	$sql = "SELECT * FROM `tbl_charterboat` WHERE client_id = '$client_id' ORDER BY id DESC ";
	return $this->db->query($sql)->result();
}
function get_chartertran_title($tran_id=NULL){
	$sql = "SELECT * FROM tbl_charterboat_detail WHERE lang = 'en' AND chartertran_id = $tran_id";
	return $this->db->query($sql)->result();
}
...
function ch_detail_distich($id1,$id2){
  $sql = "SELECT * FROM tbl_charterboat_detail where chartertran_id = $id1 AND lang = '$id2'";
  return $this->db->query($sql)->num_rows();
}

function get_chartertran_detail($tran_id=NULL){
	$sql = "SELECT tbl_charterboat.id AS tran_id ,tbl_charterboat.*,tbl_charterboat_detail.* FROM tbl_charterboat JOIN tbl_charterboat_detail on tbl_charterboat.id = tbl_charterboat_detail.chartertran_id WHERE tbl_charterboat.id = $tran_id";
	return $this->db->query($sql)->result();
}
  //--------------------------- 
...
if($this->ch_tran_distich($val_route_id)==0){
	...
	if($this->db->insert('tbl_charterboat', $data)){			
   ...
   $data = array(
      ...
      if($this->db->update('tbl_charterboat', $data)){ 
             return $val_route_id;
      }else{$resultData=0; }
...
function Add_up_details_langroute($name,$lg,$route_id){ 
        if($this->ch_detail_distich($route_id,$lg)==0){ 
            $data = array(
             "chartertran_id" =>$route_id,
             "lang" =>$lg,
             "route_name" => $name                   
            );
               if($this->db->insert('tbl_charterboat_detail', $data)){ 
                     //$resultData=1;
                     $resultData = $route_id;
               } else {  $resultData=0;  }
            ...
            $data = array(
             "route_name" => $name              
               ...
               $this->db->where('chartertran_id',$route_id);
               $this->db->where('lang',$lg);
               if($this->db->update('tbl_charterboat_detail', $data)){ 
                     $resultData = $route_id;
               }else{ $resultData=0; }
          }
     return $resultData;
   ...
   //------------------------------------------------------   
	 function insertpriceLand( $transport=NULL,$price=NULL,$landID=NULL,$pricelandID=NULL){	
              $sql = $this->db->query("SELECT MAX(orderprice) AS nNax FROM tbl_pricelandpoat WHERE transport_id  = '".$transport."' AND landtransfer_id = '".$landID."'");
	 ...
	 $data = array(
         ...
         if($this->db->insert('tbl_pricelandpoat', $data)){				 
			//$pass=1;
         ...
         if($this->db->update('tbl_pricelandpoat', $data)){				 
	...
	function loadland($dataID=NULL){
		...
		$query = $this->db->get('tbl_pricelandpoat');
	...
	function deleteDatacharter($dataID=NULL){
			$this->db->where('chartertran_id', $dataID);	 		
                        if($this->db->delete('tbl_charterboat_detail')){
                            $this->db->where('id', $dataID);
                            $this->db->delete('tbl_charterboat');
                            $this->db->where('landtransfer_id', $dataID);
                            $this->db->delete('tbl_pricelandpoat');
                            $pass = '1';
                        }else{
                            $pass = '0';
                        }
