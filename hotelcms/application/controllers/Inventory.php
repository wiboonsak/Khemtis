...
function DoNormal_pr(){
 $hotel_id = $this->input->post('hotel_id');
 $datefind_s = $this->input->post('datefind_s');
 $datefind_e = $this->input->post('datefind_e');

        if($hotel_id!=""){
             $this->Room_iv_model->room_normal_pr($hotel_id,$datefind_s,$datefind_e);
        }
...
function Doqtr(){
   ...
   $datefind_e = $this->input->post('datefind_e');
        if($hotel_id!=""){
             $this->Room_iv_model->room_qtr($hotel_id,$datefind_s,$datefind_e);
        }
}
...
function  save_gala(){
  ...
  $price_gala  = $this->input->post('price_gala');
  $txt_date  = $this->input->post('txt_date');
  $price = $this->input->post('price');

  $save = $this->Room_iv_model->save_gala_m($room_id,$txt_detail,$price_gala,$txt_date,$price);
  echo $save;
}
 ...
 function Doup_qta_iv(){
 	    ...
 	    $def_room = $this->input->post('def_room');
 	    $pdate = $yy."-".$mm."-".$dd; 
 	    if($room_id!=""){
  	          $Result = $this->Room_iv_model->Add_up_qta_iv($room_id,$qta,$pdate,$remain,$def_room);
              echo $Result;
        }else{echo 0;}
 }
