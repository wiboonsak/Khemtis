...
$hotel_q = "(SELECT concat(sum(DISTINCT tbl_pre_booking.net_price),':',sum(DISTINCT tbl_pre_booking.total_comm)) FROM  tbl_pre_booking_room_detail 
...
$tran_q = "(
SELECT  concat(sum(tbl_pre_transport_booking_detail.total_price),':',sum(tbl_pre_transport_booking_detail.total_comm)) FROM tbl_pre_transport_booking_detail 
...
$pack_q = "(SELECT  concat(sum(total_price),':',sum(total_comm)) FROM  tbl_package_pre_booking $ciq_p  WHERE  tbl_package_pre_booking.booking_status='P' AND tbl_gp_booking.add_date='$new_day' ) as n_p";

$res_q = "(SELECT concat(sum(tbl_res_pre_booking.total_price),':',sum(tbl_res_pre_booking.total_comm)) FROM tbl_res_pre_booking  $ciq_r WHERE tbl_res_pre_booking.booking_status='P' AND tbl_gp_booking.add_date='$new_day' ) as n_r";

if($service=="Hotel"){
$pack_q  = "concat(0,':',0) as n_p";
$tran_q = "concat(0,':',0) as n_t";
$res_q = "concat(0,':',0) as n_r";
}elseif ($service=="Package") {
$hotel_q = "concat(0,':',0) as n_h";
$tran_q = "concat(0,':',0) as n_t";
$res_q = "concat(0,':',0) as n_r";
