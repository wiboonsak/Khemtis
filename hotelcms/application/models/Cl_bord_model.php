...
function getadmin_proof($tak1,$tak2){
         $query = $this->db->query("
                                SELECT admin_user_id,user_full_name,admin_type,user_name,user_password
                                FROM  tbl_admin_users 
                                WHERE md5(TO_BASE64(`user_name`))='$tak2' 
                                AND admin_status = 'A' 
                                AND   md5(TO_BASE64(`user_password`))='".$tak1."'");
$row = $query->row();
if($query->num_rows()==1 && (isset($row))){
    $data = array(
                    'hotel_admin_id' => $row->admin_user_id,
                    'hotel_admin_full_name' => $row->user_full_name,
                    'hotel_admin_type' => $row->admin_type,
                    );
    $this->session->set_userdata($data);
...
/* GET Session */
  public function check_login_admin($txt,$tak1,$tak2){
  $chk_admin = $this->getadmin_proof($tak1,$tak2);
  if($chk_admin>0){
         $query = $this->db->query("
SELECT DISTINCT  tbl_users.user_id,tbl_users.client_id,tbl_users.user_full_name,tbl_users.admin_type,tbl_users.user_name,tbl_clients.user_prefix
From tbl_users 
JOIN tbl_clients ON tbl_clients.client_id = tbl_users.client_id
JOIN tbl_services ON tbl_services.client_id = tbl_clients.client_id 
JOIN tbl_hotels ON tbl_hotels.client_id = tbl_clients.client_id 
 WHERE tbl_hotels.hotel_id =$txt");
$row = $query->row();
if($query->num_rows()==1 && (isset($row)))
{
...
JOIN tbl_services ON tbl_services.client_id = tbl_clients.client_id 
                                WHERE tbl_users.user_name ='$username'
                                  ...
                                  AND tbl_services.service_cd = 'HOTEL'
