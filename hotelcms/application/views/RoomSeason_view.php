...
<font style="font-size: 18px;font-weight: bold;color:#ffffff;">Title: &nbsp;<font color='#d9efff'><?php echo$get_name_room?></font>&nbsp;</font><font style="font-size: 18px;color:#ffffff;"></font>
 ...
 <div class="row" style="margin-top: 20px;width:100%">
                                    ...
                                    <div class="row">
                                      ...
                                      <?php $i=0;
                                          ...
                                          $arddd =  explode(',',$d3);
                                             ...
                                             if($d3!=""){$bgcolor="#00a943";}
                                      ?>
                                                <li class="nav-item button">
                                                    <a href="#tab_<?php echo$i?>" onclick="get_room_season(<?php echo$arddd[0]?>,<?php echo$get_val_room_id?>,<?php echo$i?>)" data-toggle="tab" class="<?php echo$act?>" style="color:<?php echo$bgcolor?>"> <?php echo$season_nm?> </a>
                                                </li>
                                      <?php }?>
                                            </ul>
                                        </div>
  ...
  if(count($ard1)==2 && count($ard2)==2){
     $val_d1 = $ard1[0];
     $val_d2 = $ard2[1];
        $val_txt1 = explode("-",$val_d1);
        $val_txt2 = explode("-",$val_d2);
  }else{
    $val_d1 = $ard1[0];
    $val_d2 = $ard2[0];
        $val_txt1 = explode("-",$val_d1);
...
$start_day = $val_txt1[0];
$start_month = $val_txt1[1];

$end_day = $val_txt2[0];
$end_month = $val_txt2[1];
...
/*
             ...
             <input type="hidden" id="idrun_s<?php echo$i?>" name="idrun_s<?php echo$i?>" value="<?php echo$d3?>">                                    
   ...
   <div class="col-md-12 col-sm-12" style="padding:0px!important">
    ...
    <ul class="nav nav-tabs">        
      <?php $b=0;
        ...
        if($name_tab=="THB"){$txt_color="color:#00b2c5;";}
              ...
              <div class="">
                <div class="card-body row">
                        ...
                        <div class="col-lg-12 p-t-10 text-center"> 
                            <button type="button" onclick="submit_data('form_<?php echo$i?>',<?php echo$season_id?>,<?php echo$i?>)" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect m-b-10 m-r-20 btn-pink">Submit</button>
...
function submit_data(elm,season_id,ids){
   var rmd = $('#val_room_id').val();
   var form_data = $('#'+elm).serialize();
   var season_id = season_id;

            $.ajax({
                    url: "<?php echo base_url('RoomSeason/Doadd_room_season')?>",
                    type: "POST",
                    data: ({form_data:form_data,rmd:rmd,season_id:season_id,ids:ids}),
                    dataType: "html",
   success:function(data) {
      if(data!=0){
...
/*
    $.post("<?php //echo base_url('RoomSeason/Doadd_room_season')?>", 
      {form_data:form_data,room_id:room_id,season_id:season_id,ids:ids},
      function(result){
