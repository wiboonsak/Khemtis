...
<form id="form_place" class="form-horizontal" enctype="multipart/form-data">   
<?php  
            /*$type_comd = "ADD";
            $p_room_id="";
		...
		if($place_id !=0){		
                   ...
                   foreach($get_place as $value_place){
                        $a_lang=  array();
                            $val_place_id = $value_place->place_id;                         
                            $p_province_id = $value_place->province_id;
                            
                           array_push($a_lang,
                            $value_place->lang,
                            $value_place->place_title
                          );
                           array_push($txt_lang,$a_lang);  
                    }  
			if($p_province_id !=''){				
				$province = $this->Place_model->get_country_id($p_province_id,'2');
				foreach($province as $province2){}
				$dis_prov = $province2->province_nm;
				$dis_cty = $province2->country_nm;
			}			
			
		} else { 
			
			$val_place_id = $place_id;
            $p_country_id = '';
            $p_province_id = '';
...
<input type="hidden" id="val_place_id" name="val_place_id" value="<?php echo $val_place_id?>"> 
...
<font style="font-size: 18px;font-weight: bold;color:#6f6f6f;">Place Details <?php echo $naem_lang ?></font>
</div>
                        <div class="col-lg-12 p-t-10"> 
                          <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label txt-full-width">
                               <input class="mdl-textfield__input" value="<?php echo $boat_fld_nm?>" type="text" name="place_title<?php echo $fld_valu?>" id="place_title<?php echo $fld_valu?>">
                               <label class="mdl-textfield__label">Place title</label>
                          </div>
                        </div>
...
function submit_data(){	
	...
	var place_id = $('#val_place_id').val();
	var formdata = $('#form_place').serialize();
		$.post("<?php echo base_url('PlaceManage/insertUpdate_place')?>", { formdata : formdata } ,
     function(result){ 
		   if(result !=0){ 
			var link = '<?php echo base_url('PlaceManage/PlaceDetail/')?>'+result;   
