...
.hov_ch{
  ...
  border-style:solid;width: 100%;border-width:1px;padding:4px!important;border-color:#ffffff;
...
.slcolor_fix{
   ...
   text-align:left!important;height:30px!important;padding:2px!important;padding-left: 10px!important;color:#5d5d5d!important	;border-style: solid;border-width: 1px!important;
      background-image: linear-gradient(#ffffff, #f1f1f1,#ffffff,#eaeaea)!important;
...
<div class="col-md-3" style="padding-bottom:5px;">
      <label style="font-size: 18px;font-weight: bold;">FIND RESTURANTS TOURS</label>
</div>
<input type="hidden" id="type_page" value="2">

<div class="col-md-7" style="padding-bottom:5px;">
<div class="input-group">
  <input type="text" name="txtfind" id="txtfind"   class="form-control" aria-label="Amount (to the nearest dollar)" 
  ...
  $p_detail = "";
        ...
        $i=0;
        $p_pdf = "";
     foreach ($get_detail_res  as $value_detail) {
        $imgs = "http://packagecms.khemtis.com/".$value_detail->img_name;
        $p_title = $value_detail->res_name;;
        $p_detail = $value_detail->res_desc;
        $p_pdf =  $value_detail->res_pdf;
...
<div style="height:auto;border:0px solid #cccccc">
...
<?php 
$act = 0;
$idget = $this->input->get("res_id");
$variable_img = $this->Page_model->get_img_res($idget);

foreach ($variable_img as $vlimg) {$act++;$txt_act="";
   $imgm=$vlimg->img_name;
  if($act==1){$txt_act="active";}
 ?>
      <div class="item <?php echo$txt_act?>"> <img src="http://rescms.khemtis.com/<?php echo$imgm ?>"> </div>
<?php }?>
...
<?php
$act=0;$idac=-1;
  foreach ($variable_img as $vlimg) {$act++;$txt_act1="";$txt_act2="";$idac++;
   $imgmd=$vlimg->img_name;
  if($act==1){$txt_act1="<div class='item active'>";$txt_act2="</div>";}
 ?>

   <?php echo$txt_act1?>
   <div style="overflow:hidden; height:80px;margin-bottom:5px;" data-target="#carousel" data-slide-to="<?php echo$idac?>" class="col-md-3"><img src="http://rescms.khemtis.com/<?php echo$imgmd ?>" style="border:1px solid #000000;cursor: pointer;"></div>  
 
 <?php
 //if($act==4){$act=1;echo"</div><div class='item'>";}  }
    }
 ?>
...
<div class="panel polaroid_con" style="width:100%;">
  <header class="panel-heading" style="padding:5px!important;background-image: linear-gradient(#ffdd00, #ffdd00,#e2cb36,#e2d474)!important;color:#3c3c3c;font-size:15px;font-weight:bold;">
Select date resturants book
  </header>        
<div>
<?php
...
<div class="row">
  ...
  style="text-align:left!important;border-left-style:none;border-width: 1px;border-color:#b59d00;padding-top:10px;border-bottom-style: none;border-top-style: none;">
                            <div class="input-group date" id="startdate_pack" style=" border:1px solid  #eeeeee">
...
// window.location = window.location.href+"&book_st=1";
}
...
load_fillter('<?php echo$def_add_day?>');
  function load_fillter(dbook){
    //alert(dbook);
      ...
      var dbook = 0;
  $.ajax({
       url: "<?php echo base_url('ResturantsDetail/showDatares')?>",
             type: "POST",
             data: ({dbook:dbook,res_id:res_id,namepack:namepack,typeid:typeid,adults:adults,child:child}),
             dataType: "html",
