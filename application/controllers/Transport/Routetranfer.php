...
class Routetranfer extends CI_Controller {
...
$data["para_url"] = $url_txt;
        ...
        $this->load->view('Transport/Routetranfer_view');
    ...
    //--------------------------------------------
 	public  function placedataupdate(){
		...
		$result = $this->Routetranfer_model->placedataupdate($changeValue);?>
...
$data["lname_member"]=$this->session->userdata('lname_member'); 
		...
		if(!isset($data['routedata'])){
        	...
        	$this->load->view('Transport/trip_list',$data);
            $this->load->view('footer');
		}
...
//************************************************************
	...
	public function timetable_detail(){
		...
		$data['TimetableID'] = $this->input->post('TimetableID');
		$data['locationFrom'] = $this->input->post('locationFrom');
		$data['locationTo'] = $this->input->post('locationTo');
		$data['Adults1'] = $this->input->post('Adults1');
		$data['Children1'] = $this->input->post('Children1');
               
                $route_id = $this->Routetranfer_model->idroute($data['RouteID']);
                foreach ($route_id->result() as $Data){}
                $beginplace = $Data->begin_place_id;
                $destination_place = $Data->destination_place_id;
                $data['beginplace'] = $this->Routetranfer_model->list_placeData($beginplace);
                $data['destination_place'] = $this->Routetranfer_model->list_placeData($destination_place);
                $data['detailRoute']=$this->Routetranfer_model->getRouteDetail($data['RouteID'],$data['TimetableID']);	
			$this->load->view('Transport/RouteDetail',$data);
		}
